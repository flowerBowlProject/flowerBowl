...
const AddressSearch = (props) => {
  ...
  const searchAddress = () => {
    ...
    .then(res => {
      ...
      setAddress((address) => ({ ...address, lesson_longitude: data.address.x }));
      setAddress((address) => ({ ...address, lesson_latitude: data.address.y }));
  ...
  useEffect(() => {
    document.getElementById("main_address").addEventListener("click", handleAddressClick);

    return () => {
      document.getElementById("main_address").removeEventListener("click", handleAddressClick);
    }
  }, []);
