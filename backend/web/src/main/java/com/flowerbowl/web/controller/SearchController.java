...
//    public ResponseEntity<? super AllResponseDto> search(@PathVariable String query){
    public ResponseEntity<? super SearchAllResponseDto> searchAll(@PageableDefault(page = 1, size = 4) Pageable pageable, @RequestParam(value = "keyword", required = false) String keyword){
        return searchService.searchAll(pageable, keyword);
    }
    // 레시피 검색
    @GetMapping(value = "/recipes")
    public ResponseEntity<? super SearchRecipeResponseDto> searchRecipe(@PageableDefault(page = 1, size = 8) Pageable pageable, @RequestParam(value = "keyword", required = false) String keyword){
        return searchService.searchRecipe(pageable, keyword);
    }
    // 클래스 검색
    @GetMapping(value = "/lessons")
    public ResponseEntity<? super SearchLessonResponseDto> searchLesson(@PageableDefault(page = 1, size = 8) Pageable pageable, @RequestParam(value = "keyword", required = false) String keyword){
    ...
    public ResponseEntity<? super SearchCommunityResponseDto> searchCommunity(@PageableDefault(page = 1, size = 8) Pageable pageable, @RequestParam(value = "keyword", required = false) String keyword){
