...
import com.flowerbowl.web.repository.m.*;
...
public class LessonService {
    ...
    private final MuziUserRepository muziUserRepository;
    private final MuziPayRepository muziPayRepository;
    private final JpaMuziPayRepository jpaMuziPayRepository;
    ...
    // 클래스 등록
    // JWT 토큰 parsing해서 user_no의 정보를 따로 얻어와야함
    @Transactional
...
//            ResponseDto responseDto = new ResponseDto();
    ...
    public ResponseEntity<ResponseDto> lessonDelete(Long lesson_no){
        try{
            if(!jpaDataLessonRepository.existsLessonByLessonNo(lesson_no)){
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ResponseDto("FA","해당하는 lesson_no을 가진 클래스가 없습니다."));
            }
            Lesson lesson = jpaDataLessonRepository.findLessonByLessonNo(lesson_no);
    ...
    public ResponseEntity<? super FindAllResponseDto> findAll(Long user_no, Pageable pageable){
        try{
            List<LessonShortDto> list = jpaDataLessonRepository.findAllByOrderByLessonNoDesc(pageable)
                    .map(LessonShortDto::guestFrom).getContent();
            for(LessonShortDto tmp : list){
                Long lesson_no = tmp.getLesson_no();
                Long likes_no = lessonLikeRepository.countLessonLikeByLesson_LessonNo(lesson_no);
                tmp.setLesson_likes_num(likes_no);
                boolean like_status = lessonLikeRepository.existsByUser_UserNoAndLesson_LessonNo(user_no, lesson_no);
    ...
    public ResponseEntity<? super FindAllResponseDto> findAllGuest(Pageable pageable){
        try{
            ...
            for(LessonShortDto tmp : list){
                Long likes_num = lessonLikeRepository.countLessonLikeByLesson_LessonNo(tmp.getLesson_no());
...
//    public void findOneResponseDto(Long lesson_no){
    ...
    public ResponseEntity<? super PaymentInfoResponseDto> buyLesson(Long lesson_no, Long user_no){
        try{
            ...
            Pay pay = new Pay();
            // user
            User user = muziUserRepository.findUserByUserNo(user_no);
            pay.setUser(user);
            // lesson
            Lesson lesson = lessonsRepository.findByLesson_no(lesson_no);
